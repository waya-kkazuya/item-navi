...
const emit = defineEmits(['fetch-consumableItems'])
...
const decreaseStock = item => {
  if (confirm('本当に出庫処理をしますか？')) {
      const response = decreaseForm.put(`/decreaseStock/${item.id}`, {
        onSuccess: () => {
          closeModal()
          emit('fetch-consumableItems') // 親コンポーネントにイベントを発火
        },
        onError: (errors) => {
...
<template>
            ...
            <form @submit.prevent="decreaseStock(item)">
              ...
              <div class="p-2 w-full">
                  ...
                  <div class="flex items-center">
                    ...
                    <div id="unit" name="unit" class="w-1/2  text-base outline-none text-gray-700 py-1 px-3 leading-7">
                      {{ item.unit.name }}
                    </div>                        
                  </div>
                  <div v-if="localErrors.quantity" class="font-medium text-red-600">{{ localErrors.quantity }}</div>
              </div>
              <div class="mt-4 p-2 w-full">
                <button :class="{
                    'flex mx-auto text-white bg-orange-500 border-0 py-2 px-8 focus:outline-none hover:bg-orange-600 rounded': item.stock !== 0,
                    'flex mx-auto text-white bg-gray-500 border-0 py-2 px-8 focus:outline-none cursor-not-allowed rounded': item.stock === 0
                  }"
  ...
  <button @click="openModal" type="button" data-micromodal-trigger="modal-1" href='javascript:;'
  class="flex mx-auto text-white bg-sky-500 border-0 py-2 px-4 focus:outline-none hover:bg-sky-600 rounded">
