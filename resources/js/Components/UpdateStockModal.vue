          ...
          <form @submit.prevent="updateStock(item)">
            <div>
              ...
              <div class="p-2 w-full">
                ...
                <div>
                  <input type="text" id="operator_name" name="operator_name" v-model="form.operator_name" class="w-1/2 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                  <div v-if="errors.operator_name" class="font-medium text-red-600">{{ errors.operator_name }}</div>       
                </div>
              </div>
              <div class="p-2 w-full">
                  <label for="name" class="leading-7 text-sm text-blue-900">
            ...
            <div class="p-2 w-full">
              <button class="flex mx-auto text-white bg-sky-500 border-0 py-2 px-8 focus:outline-none hover:bg-sky-600 rounded">出庫処理をする</button>
            </div>
